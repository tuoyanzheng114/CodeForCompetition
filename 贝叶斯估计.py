...
def bayes():
    # 加载数据集
    df = pd.read_csv("农村收支.csv", encoding="utf-8")
    """
    在这里加载csv数据集，把括号中的路径改成csv的路径
    """
    print(df.columns.tolist())   # 查看有哪些列
    # 把数值拼成“文本”
    df["text"] = df.apply(lambda row: 
    f"{row['Region_CN']} 收入{row['Disposable_Income']} 工资{row['Wages_and_Salaries']} "
    f"经营{row['Business_Income']} 财产{row['Property_Income']} 转移{row['Transfer_Income']} "
    f"消费{row['Consumption_Expenditure']} 食品{row['Food_Tobacco_Alcohol']} "
    f"衣着{row['Clothing']} 居住{row['Housing']} 交通{row['Transport_Communication']} "
    f"教育{row['Education_Culture']} 医疗{row['Healthcare']}",
    axis=1
)
    ...
    df["target"] = df["Disposable_Income"].apply(get_label)

    feature = df["text"].tolist()
    target = df["target"].tolist()
    print("特征：", len(feature))
    print("目标：", len(target))
    print("目标值的含义：", df["target"].unique())
    # 文本特征抽取
    tf = TfidfVectorizer()
